...
## The hero on a phone (added 2026-09-11)

Under 860px the hero is the whole first screen: `min-height: 100svh` (svh, so the address bar never pushes the actions off), the name at the top, the label, lead and actions anchored at the bottom against the marquee, and the badge afloat in the free height between them, right-aligned. The fold is the marquee's top edge; nothing below it shows until the reader scrolls. Before this the hero ended where its content ended and the marquee and the dark index sat mid-screen on a phone, which the user flagged from a screenshot.
